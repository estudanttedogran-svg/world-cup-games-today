...
- The next recommended action is Phase 15F-4: a small audited UTC-conversion sample before extracting all 104 matches, or an official structured schedule source search if authorized.
...
- `src/data/real/matches.real.draft.json`: does not exist yet. Phase 15F-3 only documented conversion rules. Future PDF times must be interpreted as `America/New_York`, not stadium-local time, because the FIFA PDF declares all times as ET. Phase 15F-4 should validate a small sample before any 104-match extraction.
