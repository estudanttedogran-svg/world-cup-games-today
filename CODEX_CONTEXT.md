...
- Phase 15F-7b: applied small post-audit corrections on 2026-05-13. `src/data/real/matches.sample.draft.json` now includes boolean `conversion_audit.utc_date_shift` on all 5 sample rows. In `src/data/real/matches.real.draft.json`, `match-078` English home label was normalized from `Group E runners-up` to `Group E runners up` to match the FIFA article/API wording. No times, dates, UTC values, venues, public JSONs, MOCK notices, or app imports were changed.
- Phase 15G-0: created `REAL_DATA_PROMOTION_PLAN.md` on 2026-05-13. The plan documents that the real drafts now exist as a complete coordinated set for planning (`teams.real.draft.json`, `groups.real.draft.json`, `matches.real.draft.json`), compares their schemas with the current public JSONs and `src/types/index.ts`, identifies internal audit fields to remove before promotion, defines a temporary `_mock` strategy, plans `public/data/live-data.json` with 104 scheduled matches and zero real scores, and sets the acceptance checklist for a future 15G-1. No public JSON was changed, no real data was promoted, no `src/data/real` import was added, and no build was run.

Next allowed action depends on user authorization:

- The next recommended action is a read-only 15G-0 audit before committing the planning documents, then explicit authorization for the future coordinated promotion step. Do not promote real data or remove MOCK notices.
...
- `src/data/real/matches.sample.draft.json`: small 15F-4 sample only, not imported by the app. It contains 5 sample matches with ET-to-UTC audit metadata. As of 15F-6b, match draft rows do not contain `status` because `status` is not part of the current `Match` interface. As of 15F-7b, all 5 rows include boolean `conversion_audit.utc_date_shift`.
...
- `src/data/real/matches.real.draft.json`: full 15F-6 draft only, not imported by the app. It contains 104 matches, with 72 confirmed group-stage fixtures and 32 partial knockout fixtures. As of 15F-6b, `match-075` is corrected to 21:00 ET / `2026-06-30T01:00:00Z` and match draft rows do not contain `status`. As of 15F-7b, `match-078` English home label is normalized to `Group E runners up`. PDF times are interpreted as `America/New_York`, not stadium-local time, because the FIFA PDF declares all times as ET. This draft requires audit before any commit/promotion decision.
...
- The complete real draft set now exists for promotion planning only: 48 teams, 12 groups, and 104 matches. The next planned data step is coordinated promotion as documented in `REAL_DATA_PROMOTION_PLAN.md`; no promotion has been performed yet.
